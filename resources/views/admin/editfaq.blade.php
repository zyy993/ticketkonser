...
<main class="max-w-2xl mx-auto px-4 py-6">
    ...
    <form action="{{ route('faq.update', $faq->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="judul" value="{{ $faq->judul }}" class="w-full border p-2 rounded" required>
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-1">Deskripsi</label>
            <textarea name="deskripsi" rows="3" class="w-full border p-2 rounded" required>{{ $faq->deskripsi }}</textarea>
        </div>

        <div class="mb-4">
            <label class="block font-semibold mb-1">Deskripsi (English)</label>
            <textarea name="deskripsi_en" rows="3" class="w-full border p-2 rounded italic" required>{{ $faq->deskripsi_en }}</textarea>
        </div>

        <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded">Update</button>
        <a href="{{ route('faq.manage') }}" class="ml-2 text-sm text-gray-600 hover:underline">Cancel</a>
    </form>
</main>

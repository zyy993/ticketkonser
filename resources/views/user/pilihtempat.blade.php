...
<main class="max-w-5xl mx-auto mt-8 px-6 mb-16">
    ...
    <div class="flex justify-between gap-1.5">
        ...
        <div class="flex flex-col gap-1.5 w-1/3">
            @foreach(['ZONE A', 'ZONE B', 'ZONE D'] as $zone)
                @php
                    $colorClass = match (strtoupper($zone)) {
                        'ZONE A' => 'bg-[#a00052]',
                        'ZONE B' => 'bg-[#d6006f]',
                        'ZONE D' => 'bg-[#e9c0c9]',
                        default  => 'bg-gray-400',
                ...
                @endphp
                <div class="{{ $colorClass }} seat text-white font-bold text-sm text-center py-12 rounded-md cursor-pointer"
                     data-seat-name="{{ $zone }}"
                     onclick="selectSeat(this, '{{ $zone }}')">
                    {{ $zone }}
                </div>
            @endforeach
        </div>
        ...
        <div class="flex flex-col w-3/5 gap-1.5">
            <div class="bg-[#80003c] seat text-white font-bold text-sm text-center py-28 rounded-md cursor-pointer"
                 data-seat-name="VVIP"
                 onclick="selectSeat(this, 'VVIP')">VVIP</div>
    ...
    <!-- FORM -->
    <form action="{{ route('pilih.tempat.submit') }}" method="POST" class="mt-6 text-center">
        @csrf
...
<script>
    function getHargaByZone(zone) {
        switch (zone.toUpperCase()) {
            case 'ZONE A': return 15000;
            case 'ZONE B': return 20000;
            case 'ZONE C': return 18000;
            case 'ZONE D': return 12000;
            case 'VVIP':   return 25000;
            default: return 0;
        }
    }

    function selectSeat(element, zone) {
        const harga = getHargaByZone(zone);
  const homeId = Number('{{ $event->id }}');


        fetch(`/get-ticket-id?home_id=${homeId}&zone=${encodeURIComponent(zone)}`)
            .then(res => res.json())
            .then(data => {
                if (!data.ticket_id) {
                    alert('Tiket tidak ditemukan untuk zona tersebut.');
                    return;
                }

                document.getElementById('selected_seat_name').value = zone;
                document.getElementById('selected_harga').value = harga;
                document.getElementById('selected_ticket_id').value = data.ticket_id;

                document.getElementById('selectSeatButton').disabled = false;
                document.getElementById('hargaDisplay').innerText = "Harga Zona: Rp " + harga.toLocaleString('id-ID');

                // Highlight selected
                document.querySelectorAll('.seat').forEach(el => el.classList.remove('ring', 'ring-4', 'ring-blue-500'));
